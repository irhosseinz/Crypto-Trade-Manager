...
function exchange_orders(api,callback){
	...
	$.post("exchanger.html",'orders='+encodeURIComponent(api),function( result ) {
		...
		var data=result.result;
		$('#modal_body').html('<table class="table table-striped"><thead><tr><th scope="col">Symbol</th><th scope="col">Price</th><th scope="col">Quantity</th>'+(callback?'<th scope="col"></th>':'')+'</tr></thead><tbody></tbody></table>');
		for(let i in data){
			let title=(data[i].buy?'BUY':'SELL')+" "+data[i].symbol+" @ "+formatPrice(data[i].price);
			var tr=$('<tr>',{class:'table-'+(data[i].buy?'success':'danger')});
			tr.append('<th scope="row">'+data[i].symbol+'</th><td>'+formatPrice(data[i].price)+'</td><td>'+formatPrice(data[i].amount)+(data[i].filled?' (-'+formatPrice(data[i].filled)+')':'')+'</td>');
			if(callback){
